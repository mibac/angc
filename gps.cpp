...
void GPS::setValuesGGA(string GGA) {
  ...
  // je    this->latitude            = getCoordinates(elementVector[2]);
  this->latitude = stringToDouble(elementVector[2]);
  if (elementVector[3] == "S") this->latitude = -this->latitude;
  // je    this->longitude           = getCoordinates(elementVector[4]);
  this->longitude = stringToDouble(elementVector[4]);
...
double GPS::getCoordinates(string array) {
  ...
  // Convert input array into two sub arrays containing the degrees and the
  // minutes Check for correct array length
