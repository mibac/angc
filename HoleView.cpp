...
void HoleView::makeCurrentPointList(int h) {
  ...
  ngc->hole[h].rotCurrentPoint = ngc->hole[h].rotatePoint(
      ngc->hole[h].currentPoint, ngc->hole[h].currentUnit);
  ...
  xp = ngc->hole[h].scale *
       (ngc->hole[h].rotCurrentPoint.v[0] - ngc->hole[h].xminmax.v[0]);
  yp = ngc->hole[h].scale *
       (ngc->hole[h].rotCurrentPoint.v[1] - ngc->hole[h].yminmax.v[0]);
  xp += ngc->hole[h].xtran;
...
void HoleView::makeCurrentHoleList(int h) {
  ...
  // cout << xsize << " " << ysize << " " << xcen << " " << ycen << " " << xtran
  // << " " << ytran << endl;
  glDeleteLists(h, 1);
  ...
  for (k = 0; k < ngc->hole[h].featureNum; k++) {
    for (j = 0; j < ngc->hole[h].feature[k].polyNum; j++) {
      switch (ngc->hole[h].feature[k].featureType) {
        case 1:
          glColor3d(76.0 / 255.0, 155.0 / 255.0, 0.0);  // Tee Box
          break;
        case 2:
          glColor3d(102.0 / 255.0, 204.0 / 255.0, 0.0);  // Fairway
          break;
        case 3:
          glColor3d(0.0, 204.0 / 255.0, 0.0);  // Green
          break;
        case 4:
          glColor3d(1.0, 1.0, 204.0 / 255.0);  // Trap
          break;
        case 5:
          glColor3d(0.0, 102.0 / 255.0, 0.0);  // Trees
          break;
        case 6:
          glColor3d(51.0 / 255.0, 153.0 / 255.0, 1.0);  // Water
          break;
        case 7:
          glColor3d(0.3, 0.3, 0.3);  // Cart Path
          break;
      }
      glBegin(GL_POLYGON);
