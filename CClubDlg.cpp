...
CClubDlg::CClubDlg(int X, int Y, int W, int H, const char *L)
  ...
  const int kboxHoleX = 240 - kNavBtnSz/2;
  const int kNavBtnDX = 100;
  const int kbtnPrevX = kboxHoleX - kNavBtnDX;
  const int kbtnNextX = kboxHoleX + kNavBtnSz + kNavBtnDX;

  ////
  const int kClubcardY = kNavBtnY + kNavBtnSz + kDY;
  const int kClubcardH = 300;
  const int kClubcardW = 456;
  ////
...
int h = gCurrentHole - 1;
validDistances = getValidDistancesCount(h);
